    ...
    <form action="{{ route('asistencias.update', $asistencia->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="id_clase" class="form-control" id="id_clase" required>
                ...
                <option value="{{ $clase->id }}" {{ $clase->id == $asistencia->id_clase ? 'selected' : '' }}>{{ $clase->id }}</option> {{-- Asegúrate de que el campo `nombre` exista en el modelo `Clase` --}}
                @endforeach
            </select>
        </div>
        <div class="mb-3">
            ...
            <select name="id_alumno" class="form-control" id="id_alumno" required>
                ...
                <option value="{{ $tutorAlumno->id }}" {{ $tutorAlumno->id == $asistencia->id_alumno ? 'selected' : '' }}>{{ $tutorAlumno->alumno->nombre1 }} {{ $tutorAlumno->alumno->apellido1 }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
